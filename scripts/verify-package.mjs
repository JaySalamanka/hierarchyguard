...
const temporary = await mkdtemp(resolve(tmpdir(), "assettree-ci-package-"));
try {
  const packed = runNpm(
  ...
  await writeFile(
    ...
    `${JSON.stringify({ name: "assettree-ci-clean-install", private: true }, null, 2)}\n`,
  ...
  const installedRoot = resolve(consumer, "node_modules", "@jaysalamanka", "assettree-ci");
  ...
  if (`${smoke.stdout ?? ""}\n${smoke.stderr ?? ""}`.includes("ASSETTREE_NETWORK_EGRESS_BLOCKED")) {
